...
// Той самий захист для вентилятора: якщо бекенд "замовк" (мережа впала,
// процес завис тощо), вентилятор не повинен дути нескінченно — команда setFan()
// не повторюється, доки не прийде наступна MQTT-команда. Локальний контролер на
// бекенді (AiAgronomistService.RunLocalControlAsync) перепідтверджує рішення
// щохвилини LocalControlIntervalMinutes (типово 10 хв) незалежно від того,
// змінилось воно чи ні — тож 15-хвилинний ліміт дає запас поверх цього циклу
// й водночас гарантує, що при реально мертвому бекенді вентилятор згасне за
// 15 хв, а не годину.
