...
- **AiAgronomistService** (`Services/AiAgronomistService.cs`) — a `BackgroundService` running two independent loops via `Task.WhenAll`. **Profile supervision loop**: roughly once a day (`AiAgronomistOptions.ProfileAnalysisIntervalMinutes`), or earlier if telemetry drifts persistently outside the current range (`DetectSustainedAnomalyAsync`, gated by `MinMinutesBetweenCycles`/`SustainedExcursionMinutes`), it reads `TelemetryRecord`s from the last `TrendWindowMinutes`, a summary of what the local controller did since the last review, fetches a JPEG from the ESP32-CAM, and sends all of it to the Gemini `generateContent` API asking it to (re)set the ideal parameter ranges for the plant — persisted as a `PlantProfile` row (temp/humidity/soil-moisture ranges, daily light-hours target, freeform notes). **Local control loop**: every `LocalControlIntervalMinutes` (no AI call), it reads the current `PlantProfile` plus recent telemetry and decides pump/fan/light with simple rules (sustained-reading fan threshold, declining-trend-plus-cooldown pump rule, ambient-Lux-shortfall light rule), logs an `AiDecisionRecord`, and republishes the resulting `AiCommand` via `IMqttPublisher` to `MqttOptions.CommandsTopic` every tick regardless of change (the ESP32's `FAN_MAX_RUNTIME_MS`/pump failsafes rely on that repetition).

- **Models/** — plain POJOs/records split between MQTT wire types (`TelemetryMessage`, `AiCommand` — `JsonPropertyName`-annotated snake_case to match the ESP-32/MQTT side), the EF entities (`TelemetryRecord`, `AiDecisionRecord`, `PlantProfile`), the Gemini response DTOs (nested private records inside `AiAgronomistService`), and the `*Options` config-binding classes described above.
